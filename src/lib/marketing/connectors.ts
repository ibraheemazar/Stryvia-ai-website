...
export function providerConfigured(provider: string): boolean {
  switch (provider) {
    ...
    case "search_console":
      return !!process.env.SEARCH_CONSOLE_SITE && !!process.env.GOOGLE_SERVICE_ACCOUNT_JSON;
    default:
      // Ad/social platforms wired via their own OAuth apps; reported by env flag.
...
export async function slackNotify(text: string): Promise<boolean> {
  ...
  try {
    ...
    console.error("[stryvia] slack notify failed:", err);
    return false;
  }
}

// WhatsApp via the Meta Cloud API (also works with Unifonic by swapping the
// endpoint). Sends a plain text message to an E.164 number.
export async function sendWhatsApp(to: string, message: string): Promise<boolean> {
  ...
  try {
    ...
    if (!res.ok) console.error("[stryvia] whatsapp failed:", res.status, await res.text());
    return res.ok;
  } catch (err) {
    console.error("[stryvia] whatsapp error:", err);
    return false;
  }
}

// SMS via Twilio (Unifonic supported by changing the endpoint/auth).
export async function sendSMS(to: string, message: string): Promise<boolean> {
  ...
  try {
    ...
    if (!res.ok) console.error("[stryvia] sms failed:", res.status, await res.text());
    return res.ok;
  } catch (err) {
    console.error("[stryvia] sms error:", err);
    return false;
  }
}
...
export async function webhookPost(url: string, payload: unknown): Promise<boolean> {
  ...
  try {
    ...
    console.error("[stryvia] webhook error:", err);
    return false;
  }
}
