...
**Tested.** Screenshots in `checkpoint-3/` (dark scheme, reduced motion, iPhone 13 + desktop, EN + AR: landing, resume, delete-my-data, interview before and after the first message). The interview shots show the visitor's first message and the failed-turn state, because the model has no credits; that state now says "Stryvia's AI is unavailable for a moment" (new `errorProvider` copy, EN + AR) instead of blaming the visitor's connection. RTL checks on the Arabic shots: mirrored progress rail, Arabic heading face, `WhatsApp` isolated inside an Arabic sentence, composer buttons on the correct side. Session hydration fails loudly with a Retry instead of a spinner when the network drops (found by the screenshot run through the sandbox proxy, which also drops JS/CSS chunks at random; the script now reloads until the page is hydrated and styled). Voice adapters are typed and shaped after each vendor's documented API but **not executed** (no keys). The Web Speech fallback is the same mechanism the admin Prompt Maker already uses.
